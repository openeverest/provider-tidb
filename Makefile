...
# TiDB Operator (v2) version whose CRDs and image the provider targets.
OPERATOR_VERSION ?= v2.0.1

# Image URL to use for building/pushing image targets
IMG ?= ghcr.io/openeverest/provider-tidb-dev:latest
...
K3D_CLUSTER_NAME ?= provider-tidb-test
...
CHART_DIR ?= charts/provider-tidb
...
install-crds: ## Install OpenEverest CRDs (and your operator's CRDs) into the cluster.
	...
	kubectl apply --server-side -f https://raw.githubusercontent.com/pingcap/tidb-operator/$(OPERATOR_VERSION)/manifests/crd/core.pingcap.com_tikvgroups.yaml
